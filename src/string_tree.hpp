...
namespace STree{
  ...
  public:
    ...
    unsigned long long int byteSize() const{return sizeof(Node) + label.size()*sizeof(char) + children.size()*sizeof(Node*);};
  };
  ...
  public:
    ...
    Node * lastFNode;
    Node * firstFactor;
    unsigned long long int read_len_tkb;
    double start_time;
    ...
    unsigned long long int byteSize(){
      unsigned long long int nodes_size = 0;
      for(size_t i = 0; i < nodes.size(); i++){
	nodes_size += this->nodes[i]->byteSize();
      }
      return sizeof(Tree) + 
	nodes_size +
	this->fnodes.size()*sizeof(this->fnodes[0]);
    }
    ...
    inline virtual Node * insertCharNode2(char c){
      if (this->stat) {
        this->stat->len_factors.push_back(1);
      }
      return this->addChild(this->root, true, std::string(1, c));
