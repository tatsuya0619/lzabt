...
void slp2enc(std::vector<std::pair<unsigned int, unsigned int> > & vars, unsigned int decompressedSize, const std::string & outputFilename, unsigned int *roots, unsigned int size){
  ...
  //std::cout << "start = " << edict->start << std::endl;
  ...
  if(size == 0){
    ...
    EncodeCFGs(edict, output, roots, size);
  }
  DestructEDict(edict);
  fclose(output);
}
