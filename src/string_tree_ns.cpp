...
Node * Tree::insertFactorNode(Node * node, unsigned int strbegin, unsigned int len){
  ...
  Node * child;
  if (strpar_begin < this->str_size) child = this->getChildBeginWith(last_node, this->getCharAt(strpar_begin));
  else child = NULL;
...
unsigned int Tree::LCPFromNode(Node * node, unsigned int strbegin, unsigned int len){
  if (len == UINT_MAX) len = (unsigned int) this->str_size;
  if (len == 0) return 0;
  if (node == NULL || strbegin >= this->str_size) return 0;
...
unsigned int Tree::LCPToNode(const Node * node, unsigned int strbegin, unsigned int len) const{
  ...
  */
  ...
  for(; (lcp < node->in_edge_len) && (strbegin+lcp < this->str_size) && lcp < len; ++lcp){
